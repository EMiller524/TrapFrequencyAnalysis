...
class Electrode_vars:
    ...
    It is in the form: DCX = [
        - RFamplitude: the amplitude of the RFfreq dependent voltage
        - RFfrequency: the RFfrequency of the voltage (this could be a electrode independent var in the future)
        - DCoffset: the offset of the voltage (DC offset)
        - amplitude:
        - frequency:
        - phase:
        ]

    the last three variables are not used yet, but the idea is they would be needed for a multi frequency analysis
    """
    ...
    def get_RFamplitude(self, electrode):
        return self.vars[electrode][0]

    def get_RFfrequency(self, electrode):
        return self.vars[electrode][1]

    def get_DCoffset(self, electrode):
    ...
    def get_frequency(self, electrode):
        return self.vars[electrode][4]

    def get_phase(self, electrode):
        return self.vars[electrode][5]

    def set_RFamplitude(self, electrode, amplitude):
        self.vars[electrode][0] = amplitude

    def set_RFfrequency(self, electrode, frequency):
        self.vars[electrode][1] = frequency

    def set_DCoffset(self, electrode, offset):
        self.vars[electrode][2] = offset

    def set_amplitude(self, electrode, amplitude):
        self.vars[electrode][3] = amplitude

    def set_frequency(self, electrode, frequency):
        self.vars[electrode][4] = frequency

    def set_phase(self, electrode, phase):
        self.vars[electrode][5] = phase
...
def get_electrodvars_w_twist(rfamp, rffreq, twist, endcaps):
    ...
    RF1 = [rfamp, rffreq, twist, 0,0,0]
    RF2 = [rfamp, rffreq, twist, 0,0,0]

    DC1 = [0, rffreq, endcaps - twist, 0,0,0]
    DC2 = [0, rffreq, -twist, 0,0,0]
    DC3 = [0, rffreq, -twist, 0,0,0]
    DC4 = [0, rffreq, -twist, 0,0,0]
    DC5 = [0, rffreq, endcaps - twist, 0,0,0]
    DC6 = [0, rffreq, endcaps - twist, 0,0,0]
    DC7 = [0, rffreq, -twist, 0,0,0]
    DC8 = [0, rffreq, -twist, 0,0,0]
    DC9 = [0, rffreq, -twist, 0,0,0]
    DC10 = [0, rffreq, endcaps - twist, 0,0,0]
    ...
    add_trap_capacitence_to_electrodvars(
        elecvars
    )  # add capacitance to the electrode variables
    return elecvars
...
def add_trap_capacitence_to_electrodvars(electrode_varss):
    ...
    rf_amp = electrode_varss.get_RFamplitude("RF1")

    for electrode in constants.electrode_names:
        prevamp = electrode_varss.get_RFamplitude(electrode)
        newamp = constants.electrode_RF_pickoff_amp_multipliers[electrode] * rf_amp
        electrode_varss.set_RFamplitude(electrode, prevamp + newamp)
